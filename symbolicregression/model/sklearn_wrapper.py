...
class SymbolicTransformerRegressor(BaseEstimator):
    ...
    def fit(self, X, Y, verbose=False):
        ...
        for input_id, candidates_id in candidates.items():
            ...
            refined_candidates = self.refine(
                scaled_X[input_id], Y[input_id], candidates_id, verbose=verbose
            )
            for i, candidate in enumerate(refined_candidates):
                if scaler is not None:
                    refined_candidates[i]["predicted_tree"] = scaler.rescale_function(
                        self.model.env,
                        candidate["predicted_tree"],
                        *scale_params[input_id],
                    )
                else:
                    refined_candidates[i]["predicted_tree"] = candidate[
                        "predicted_tree"
                    ]
    ...
    def refine(self, X, y, candidates, verbose):
        ...
        for i, candidate in enumerate(candidates):
            candidate_skeleton, candidate_constants = (
                self.model.env.generator.function_to_skeleton(
                    candidate, constants_with_idx=True
                )
            )
            if "CONSTANT" in candidate_constants:
                candidates[i] = self.model.env.wrap_equation_floats(
    ...
    def retrieve_tree(
        ...
        for idx in idxs:
            ...
            if not best_tree:
                if with_infos:
                    best_trees.append(
                        {"predicted_tree": None, "refinement_type": None, "time": None}
                    )
                else:
        ...
        if batch:
            ...
            for tree_idx in range(len(tree)):
                ...
                if tree[tree_idx] is None:
                    ...
                    numexpr_fn = self.model.env.simplifier.tree_to_numexpr_fn(
                        tree[tree_idx]
                    )
